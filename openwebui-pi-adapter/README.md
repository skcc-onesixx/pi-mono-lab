...
| `HOST` | 바인딩 주소 (Docker/외부 접근 시 `0.0.0.0`) | `0.0.0.0` |
...
2. **➕ Add Connection** 클릭
3. **API URL**: 아래 표 참고
4. **API Key**: `dummy` (또는 비워둠)
5. 필요 시 **Model IDs**에 `pi-agent` 추가

| 환경 | API URL |
|------|---------|
| Open WebUI와 Adapter 모두 로컬 | `http://localhost:3001/v1` |
| Open WebUI Docker, Adapter 호스트 | `http://host.docker.internal:3001/v1` |

Docker 사용 시 `docker-compose`에 `extra_hosts: host.docker.internal:host-gateway` 필요.

**상세 가이드**: `sixx/50-08-Open-WebUI-Connection-설정-가이드.md`
